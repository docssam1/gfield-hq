...
"""
smart_eraser.py
pix2text 레이아웃 분석 + Gemini Vision으로 손글씨만 골라서 흰색 마스킹
...
def get_layout(image_path: str) -> list:
    ...
    result = p2t.recognize_page(image_path, save_debug_res=str(output_dir))
    # layout_out.json 읽기
...
def classify_regions(model, image: Image.Image, regions: list) -> list:
    """Gemini Vision으로 각 FIGURE 영역이 손글씨인지 인쇄인지 분류"""
    if not regions:
        return []

    # FIGURE/TEXT/TITLE 영역 정보 정리
    region_desc = []
...
각 영역을 분석해서 다음 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{{"handwriting": [손글씨인 id 목록], "printed": [인쇄된 것 id 목록]}}

판단 기준:
- 손글씨: 학생이 쓴 답안, 계산 과정, 동그라미 채점 표시, 낙서
...
def process(input_path: str, output_path: str = None):
    ...
    # 2. Gemini로 손글씨 분류
    print("\n[2/3] Gemini Vision으로 손글씨 분류 중...")
    model = setup_gemini()
    image = Image.open(inp).convert("RGB")
    handwriting_ids = classify_regions(model, image, regions)
...
def main():
    parser = argparse.ArgumentParser(description="스마트 손글씨 제거 (pix2text + Gemini)")
