...
사용법:
  ...
  python3 smart_eraser.py input.jpg --output output/result.png --project PROJECT_ID
"""
...
def call_gemini_vertex(prompt: str, image: Image.Image, project_id: str) -> str:
    import io
    buf = io.BytesIO()
...
def classify_regions(image: Image.Image, regions: list, project_id: str) -> list:
    if not regions:
        return []
...
- 확실하지 않으면 printed로 분류 (인쇄 텍스트 보호 우선)"""
    ...
    text = call_gemini_vertex(prompt, image, project_id)
...
def process(input_path: str, project_id: str, output_path: str = None):
    inp = Path(input_path)
    if not output_path:
    ...
    handwriting_ids = classify_regions(image, regions, project_id)
...
def main():
    default_project = os.environ.get("GOOGLE_CLOUD_PROJECT", "gen-lang-client-0794247388")
    ...
    parser.add_argument("--project", default=default_project, help="GCP 프로젝트 ID")
    args = parser.parse_args()
    process(args.input, args.project, args.output)
